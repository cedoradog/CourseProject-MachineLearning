...
###Results
Comparing the mean and standard deviation of error measures (proportion of incorrectly classified observations in each simulation) for the random forest models with and without PCA preprocessing, the advantage of preprocessing is not evident. We choose a model without PCA preprocessing, which gives error rates between `r round(100 * (1 - max(results$accuracy)), 1)`% and `r round(100 * (1 - min(results$accuracy)), 1)`% in the `r nSimulations` simulations performed.
...
```{r final model and evaluation, cache=TRUE, echo=FALSE}
#library(randomForest)
#library(caret)
...
Finally, we use the given model to perform the classification task assigned
